...
一个小巧的 Windows 托盘程序，以饼图显示 Codex 每周使用限额的剩余比例。
...
运行本程序不会消耗模型推理 Token，也不会减少 Codex 每周使用额度。程序只调用 `account/rateLimits/read` 读取账户元数据，从不启动 Codex 对话或模型回合，也不调用 `thread/start` 或 `turn/start`。
